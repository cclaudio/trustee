...
use super::{RepositoryPlugin, RepositoryPluginBuild};
...
fn create_dirname(path: &Path) -> Result<()> {
...
fn read_file_to_vec(path: &Path) -> Result<Vec<u8>> {
    let mut file = fs::File::open(path).with_context(|| format!("Open CA crt file"))?;
    let size: u64 = file
        .metadata()
        .context(format!("Get {} metadata", path.display()))?
        .len();
    let mut v: Vec<u8> = Vec::with_capacity(size as usize);
    file.read_to_end(&mut v)
        .context(format!("Read {} to end", path.display()))?;
...
impl RepositoryPluginBuild for NebulaPluginConfig {
    ...
    fn create_plugin(
        ...
        create_dirname(&ca.work_dir.as_path())?;

        match config.ca_generation_policy {
            x if x == CaGenerationPolicy::GenerateIfNotFound as u32 => {
                if !ca.crt.exists() || !ca.key.exists() {
                    remove_file(ca.crt.as_path())?;
                    remove_file(ca.key.as_path())?;

                    create_dirname(ca.crt.as_path())?;
                    create_dirname(ca.key.as_path())?;
        ...
        Ok(Arc::new(RwLock::new(NebulaPlugin { ca }))
            as Arc<RwLock<dyn RepositoryPlugin + Send + Sync>>)
    }
}
...
impl Drop for Credential {
    fn drop(&mut self) {
        if let Err(e) = remove_file(self.crt.as_path()) {
            log::warn!("{}", e.to_string());
        }
        if let Err(e) = remove_file(self.key.as_path()) {
            log::warn!("{}", e.to_string());
        }
    }
}
...
impl NebulaCa {
    pub fn get_credential_resource(&self, params: &NebulaCredentialParams) -> Result<Vec<u8>> {
        ...
        let resource = CredentialResource {
            node_crt: read_file_to_vec(cred.crt.as_path())?,
            node_key: read_file_to_vec(cred.key.as_path())?,
            ca_crt: read_file_to_vec(self.crt.as_path())?,
        };
...
impl RepositoryPlugin for NebulaPlugin {
