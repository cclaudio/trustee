...
impl TryFrom<NebulaCaPluginConfig> for NebulaCaPlugin {
    ...
    fn try_from(config: NebulaCaPluginConfig) -> Result<Self> {
        ...
        log::info!("Nebula CA certificate: {}\n{}", crt.display(), nebula.print(&crt)?);
...
impl NebulaCertBin {
    ...
    pub fn print(&self, crt: &Path) -> Result<String> {
        ...
        if !output.status.success() {
            return Err(anyhow!("{} print {:?}", self.path.display(), &args));
        }
    ...
    pub async fn sign(&self, params: &NebulaCredentialParams, ca_key: &Path, ca_crt: &Path, node_key: &Path, node_crt: &Path) -> Result<()> {
...
mod tests {
    ...
    fn test_credential_service_params(#[case] query: &str, #[case] expected: Option<Vec<OsString>>) {
